...
export function findFileOnDisk(filename) {
  ...
  const candidates = [
    // Đường dẫn từ env var (ưu tiên cao nhất)
    ...(process.env.UPLOAD_DIR ? [path.join(process.env.UPLOAD_DIR, safeFilename)] : []),
    // Đường dẫn chuẩn Next.js
    path.join(process.cwd(), 'public', 'uploads', safeFilename),
    path.join(process.cwd(), 'uploads', safeFilename),
    // Standalone deployment: Next.js copy public vào đây
    path.join(process.cwd(), '.next', 'standalone', 'public', 'uploads', safeFilename),
    // Standalone server.js chạy từ thư mục .next/standalone
    path.join(process.cwd(), '..', 'public', 'uploads', safeFilename),
    path.join(process.cwd(), '..', '..', 'public', 'uploads', safeFilename),
    // Docker / container thường mount volume vào /app
    path.join('/app', 'public', 'uploads', safeFilename),
    path.join('/app', 'uploads', safeFilename),
    // Fallback /tmp

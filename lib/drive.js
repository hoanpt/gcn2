...
/**
 * Tìm folder của hồ sơ trên Drive theo mã hồ sơ (ví dụ CDC-123456)
 */
export async function findApplicationFolderOnDrive(applicationId) {
  const { drive, enabled, folderId: parentId } = await getDrive();
  if (!enabled || !applicationId) return null;

  try {
    const q = parentId 
      ? `name = '${applicationId}' and '${parentId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false`
      : `name = '${applicationId}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false`;
    ...
    console.error('[GoogleDrive] Lỗi tìm folder theo ID:', err.message);
  }
  return null;
}

/**
 * Tìm các file trên Drive có tên chứa mã hồ sơ hoặc từ khóa
 */
export async function searchFilesInDriveByAppId(applicationId) {
  const { drive, enabled } = await getDrive();
  if (!enabled || !applicationId) return [];

  try {
    const res = await drive.files.list({
      q: `name contains '${applicationId}' and mimeType != 'application/vnd.google-apps.folder' and trashed = false`,
      fields: 'files(id, name, mimeType, webViewLink, webContentLink, size)',
    });
    return res.data.files || [];
  } catch (err) {
    console.error('[GoogleDrive] Lỗi search files theo ID:', err.message);
    return [];
  }
}
